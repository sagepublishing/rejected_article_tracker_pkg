...
class TestArXivAuthorNames(unittest.TestCase):

    def test_convert_names_with_valid_items(self):
        names = ArXivAuthorNames(names_string='Hails, Andy Carlos;King, Helen;Day, Adam;Harris, D')
        self.assertEqual(names.names(), 'Andy Carlos+Hails, Helen+King, Adam+Day, D+Harris')

    def test_convert_names_with_one_item(self):
        names = ArXivAuthorNames(names_string='Andy Hails')
